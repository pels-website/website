export default {
  async fetch(request, env) {
    ...
    // Pass CSS, JS, images, JSON data files straight through
    ...
    assetUrl.search = '';
    const [templateRes, dataRes, contactsRes] = await Promise.all([
    ...
    // Resolve translation value, substituting {name} and {year}
    const resolve = (path) => {
      let v = get(path);
      if (typeof v === 'string') {
    ...
    const rewriter = new HTMLRewriter()
      ...
      // --- JSON-LD (fully generated, no hardcoded personal data) ---
